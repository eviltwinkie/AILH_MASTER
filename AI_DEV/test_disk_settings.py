...
DATASET_PATH = Path("/DEVELOPMENT/ROOT_AILH/DATA_STORE/TRAINING")
HEADER_SIZE = 44
SAMPLE_RATE = 4096
...
# ~24 CPU cores → test around and below core count
PREFETCH_THREADS_VALUES = [24, 22, 20, 18, 16, 12, 8, 6, 4, 2, 1]

# Queue depth between producer (disk) and consumer (GPU).
# Powers of two, covering shallow to fairly deep pipelines.
PREFETCH_DEPTH_VALUES = [131072, 65536, 32768, 16384, 8192, 4096, 2048, 1024, 896, 768, 640, 512, 384, 256, 128, 96, 64, 48, 32, 24, 16, 14, 12, 10, 8, 6, 4, 2, 1]

# How many files a single task handles from disk.
# Large values to exploit NVMe, but still step down to sane small values.
FILES_PER_TASK_VALUES = [131072, 65536, 32768, 16384, 8192, 4096, 2048, 1024, 896, 768, 640, 512, 384, 256, 128, 96, 64, 48, 32, 24, 16, 14, 12, 10, 8, 6, 4, 2, 1]

#PREFETCH_THREADS_VALUES = [ 1 ]
#PREFETCH_DEPTH_VALUES = [ 1 ]
#FILES_PER_TASK_VALUES = [ 4096 ]
