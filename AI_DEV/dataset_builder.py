...
# Restrict labels to exactly these two
...
class MultiSplitBuilder:
    ...
    def discover_global_labels(self, dirs: List[Path]) -> Tuple[List[Tuple[Path, str]], Dict[str, int]]:
        ...
        for d in dirs:
            ...
            present.update(lbl for _p, lbl in recs)
        # Enforce only allowed labels and canonical order: LEAK=0, NOLEAK=1
        label_list = [l for l in ALLOWED_LABELS if l in present]
        if not label_list:
            raise RuntimeError("No labeled WAVs found. Expected LEAK/ or NOLEAK/ subfolders.")
        label2id = {l: i for i, l in enumerate(label_list)}
        return all_records, label2id

    # --------- single split build ---------# --------- single split build ---------
