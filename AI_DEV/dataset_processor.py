...
PREFETCH_THREADS = 6      # Number of parallel disk readers
FILES_PER_TASK = 100      # Files per batch (~28MB) - TODO: Change to 340 (empirical optimal)

# Pipeline buffering
RAM_QUEUE_SIZE = 128     # Queue depth - TODO: Change to 96 (saves 55GB RAM!)
...
def run_pipeline() -> None:
    ...
    if use_memmap_mapping:
        mapping_memmap_path = str(cfg.OUTPUT_DIR / "PIPELINE_MEMMAP_TEMP.DAT")
    ...
    if use_memmap_mapping:
        ...
        # Save without extension first, then rename to uppercase .NPY
        temp_path = cfg.OUTPUT_DIR / "PIPELINE_MEMMAP_temp"
        np.save(str(temp_path), mapping_array, allow_pickle=False)
        # np.save() creates PIPELINE_MEMMAP_temp.npy, rename to PIPELINE_MEMMAP.NPY
        import shutil
        shutil.move(str(temp_path) + '.npy', str(mapping_path))
        with open(str(mapping_path), 'r+b') as f:
            os.fsync(f.fileno())
